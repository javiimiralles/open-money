...
- Two-face display typography — Manrope (weight 900, hero scale) + Inter (weight 600, sub-display scale). The contrast is the brand's typographic story.
...
### Brand & Accent
- **Green** (`{colors.primary}` — `#9fe870`): The brand's universal CTA color. Every primary button, every "Send money" pill, the brand's logo accent.
- **Green Hover** (`{colors.primary-active}` — `#cdffad`): The lighter green for active state.
- **Green Neutral** (`{colors.primary-neutral}` — `#c5edab`): A mid-saturation green used as a neutral active fill.
- **Green Pale** (`{colors.primary-pale}` — `#e2f6d5`): The lightest green for soft surface tints / badge backgrounds.
...
1. **Manrope** — open-source geometric sans with an unusually heavy weight 900 used for all hero displays. The face is the brand's typographic signature. Always at weight 900, never lighter on the marketing surface. Available on [Google Fonts](https://fonts.google.com/specimen/Manrope).
...
- Don't replace Manrope with another geometric sans for hero typography — the chosen face IS the brand's voice.
